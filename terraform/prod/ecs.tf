...
/* ecs service cluster */
resource "aws_ecs_cluster" "ppv" {
  name = "${var.ecs_cluster_name}"
}

/* create a repository */
resource "aws_ecr_repository" "ppv_ecr" {
  name = "ppv-${var.env}"
}

/* create a repository for nginx */
resource "aws_ecr_repository" "ppv_nginx" {
  name = "ppv-${var.env}-nginx"
}

/* create a repository for the Picogauge Website */
resource "aws_ecr_repository" "ppv_website" {
  name = "ppv-website"
}
